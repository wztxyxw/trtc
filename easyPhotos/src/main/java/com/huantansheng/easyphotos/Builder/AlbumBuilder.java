...
public class AlbumBuilder {
    ...
    public AlbumBuilder setSelectedPhotos(ArrayList<Photo> selectedPhotos) {
        ...
        if (selectedPhotos.size() > 0) {
            if (selectedPhotos.get(0).type.contains(Type.VIDEO)) {
                Setting.selectType = Type.VIDEO;
            } else if (selectedPhotos.get(0).type.contains(Type.PHOTO)) {
                Setting.selectType = Type.PHOTO;
            } else if (selectedPhotos.get(0).type.contains(Type.GIF)) {
    ...
    /**
     * false 单类型 true 多类型  默认true
     *
     * @param allType
     * @return
     */
    public AlbumBuilder setAllType(boolean allType) {
        Setting.allType = allType;
        return AlbumBuilder.this;
    }
