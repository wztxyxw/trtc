        ...
        ProfileManager.getInstance().getUserInfoByUserId(sponsor, new ProfileManager.GetUserInfoCallback() {
            ...
            public void onSuccess(final UserModel model) {
                if (userIdList == null || userIdList.size() == 0) {
                    ...
                    } else if (callType == ITRTCAVCall.TYPE_FLY_AUDIO_CALL) {
                        if (!TextUtils.isEmpty(mFlyUserId) && model.userId.equals(mFlyUserId) && mSourceId > 0) {
                            ARouter.getInstance().build("/fly/audio_call")
                                    .withSerializable("data", model)
                                    .withInt("sourceId", mSourceId)
                                    .navigation();
                        }
//                        ARouter.getInstance().build("/fly/audio_call")
//                                .withSerializable("data", model)
...
//                                .navigation();
    ...
    public void setFlyUserId(String flyUserId) {
        this.mFlyUserId = flyUserId;
    }
