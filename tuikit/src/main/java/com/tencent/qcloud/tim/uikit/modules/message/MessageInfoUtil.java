...
import com.tencent.qcloud.tim.uikit.config.CustomType;
            ...
            if (data.equals(MessageCustom.BUSINESS_ID_GROUP_CREATE)) {
                ...
                try {
                    ...
                    if (!TextUtils.isEmpty(messageCustom.businessID) && messageCustom.businessID.equals(MessageCustom.BUSINESS_ID_GROUP_CREATE)) {
                        ...
                        if (jsonObject.containsKey("pddCustomType")) {

                            int customType = jsonObject.getInteger("pddCustomType");

                            if (customType == CustomType.PRESENTS_TYPE) {

                                MessagePresents messagePresents = null;
                                try {
                                    messagePresents = new Gson().fromJson(data, MessagePresents.class);
                                ...
                                if (timMessage.isSelf()) {
                                    msgInfo.setExtra("送ta一个" + messagePresents.giftName);
                                } else {
                                    msgInfo.setExtra("送你一个" + messagePresents.giftName);
                                }
                            ...
                            if (callModel != null) {
                                ...
                                if (callModel. callType == ITRTCAVCall.TYPE_FLY_AUDIO_CALL) {
                                    return null;
                                } else  {
                                    if (callModel.action == CallModel.VIDEO_CALL_ACTION_DIALING) {
                                        content = isGroup ? ("\"" + senderShowName + "\"" + "发起群通话") : ("发起通话");
                                    } else if (callModel.action == CallModel.VIDEO_CALL_ACTION_SPONSOR_CANCEL) {
                                        content = isGroup ? "取消群通话" : "取消通话";
                                    } else if (callModel.action == CallModel.VIDEO_CALL_ACTION_LINE_BUSY) {
                                        content = isGroup ? ("\"" + senderShowName + "\"" + "忙线") : "对方忙线";
                                    } else if (callModel.action == CallModel.VIDEO_CALL_ACTION_REJECT) {
                                        content = isGroup ? ("\"" + senderShowName + "\"" + "拒绝群通话") : "拒绝通话";
                                    } else if (callModel.action == CallModel.VIDEO_CALL_ACTION_SPONSOR_TIMEOUT) {
                                    ...
                                    } else if (callModel.action == CallModel.VIDEO_CALL_ACTION_ACCEPT) {
                                        content = isGroup ? ("\"" + senderShowName + "\"" + "已接听") : "已接听";
                                    } else if (callModel.action == CallModel.VIDEO_CALL_ACTION_HANGUP) {
                                        content = isGroup ? "结束群通话" : "结束通话，通话时长：" + DateTimeUtil.formatSeconds(callModel.duration);
                                    } else {
                                        content = "不能识别的通话指令";
                                    }
                                    if (isGroup) {
