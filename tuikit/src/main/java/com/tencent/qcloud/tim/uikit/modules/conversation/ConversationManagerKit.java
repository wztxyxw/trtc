...
public class ConversationManagerKit implements MessageRevokedManager.MessageRevokeHandler {
    ...
    List<ConversationInfo> mNewConversationInfos = new ArrayList<>();
    public List<String> deleteList = new ArrayList<>();

    public final String FLY_AUDIO_LIST = "fly_audio_list";

    public List<String> getDeleteList(){
        if (null == deleteList || deleteList.size() == 0) {
            deleteList = mSP.getList(FLY_AUDIO_LIST, String.class);
        }
        if (null == deleteList) {
    ...
    public void addDeleteList(String userId){
        ...
        if (!list.contains(userId)) {
            ...
            for (ConversationInfo c : mNewConversationInfos) {
            ...
            if (isHave) {
                list.add(userId);
                mSP.putList(FLY_AUDIO_LIST, list);
            }
        }
        mNewConversationInfos.clear();
        deleteList = list;
    }

    public void removeDeleteList(String userId){
        List<String> list = getDeleteList();
        if (null == list) {
            list = new ArrayList<>();
        }
        ...
        V2TIMManager.getConversationManager().getConversationList(0, 100, new V2TIMValueCallback<V2TIMConversationResult>() {
            ...
            public void onSuccess(V2TIMConversationResult v2TIMConversationResult) {
                ...
                for (V2TIMConversation v2TIMConversation : v2TIMConversationList) {
                    ...
                    if (v2TIMConversation.getUserID().equals("admin")) {

                        if (!BuildConfig.DEBUG) {

                            deleteConversation("admin", false);
                            continue;
                        }
...
//
                    if (conversationInfo != null) {
                        if (!v2TIMConversation.getUserID().equals("admin") && v2TIMConversation.getUserID().equals(v2TIMConversation.getShowName())) {
                            users.add(v2TIMConversation.getUserID());
                        }
...
//                if (users.size() > 0) {
//                    getUsersInfo(users, infos, callBack);
//                } else {
...
//                }
    ...
    private void getUsersInfo(List<String> users, final List<ConversationInfo> infos, final IUIKitCallBack callBack){
            ...
            public void onSuccess(List<V2TIMUserFullInfo> v2TIMUserFullInfos) {
                ...
                for (V2TIMUserFullInfo userInfo : v2TIMUserFullInfos) {
                    ...
                    for (int i = 0; i < infos.size(); i++) {
                        if (infos.get(i).getId().equals(userInfo.getUserID())) {
                            infos.get(i).setTitle(userInfo.getNickName());
                            continue;
                        }
                    }
...
//                mProvider.setDataSource(sortConversations(infos));
    ...
    private void setDataSource(List<ConversationInfo> infos){


        List<ConversationInfo> ss = new ArrayList<>();

        if (null != deleteList) {
            for (ConversationInfo c : infos) {
                if (!deleteList.contains(c.getId())) {
                    ss.add(c);
                }
            }
        }
        mProvider.setDataSource(sortConversations(ss));
    }
    ...
    public void onRefreshConversation(List<V2TIMConversation> v2TIMConversationList) {
        ...
        for (int i = 0; i < v2TIMConversationList.size(); i++) {
            ...
            LogUtil.e(TAG, "refreshConversation v2TIMConversation " + v2TIMConversation.getUserID());
            ...
            if (conversationInfo != null) {
                ...
                if (conversationInfo.getUnRead() > 0) {
                    ...
                    mNewConversationInfos.add(conversationInfo);
                } else if (null != conversationInfo.getLastMessage()) {
                    if (conversationInfo.getLastMessage().isSelf()) {

                        infos.add(conversationInfo);

                        mNewConversationInfos.add(conversationInfo);
                    } else if (conversationInfo.getLastMessage().getTimMessage().getElemType() > 0) {
    ...
    public void setConversationTop(int index, ConversationInfo conversation) {
        ...
        setDataSource(mProvider.getDataSource());
...
//        mProvider.removeConversation(id);
//    }
//
//    public void addConversation(String id) {
//        mProvider.addConversation(id);
//    }
//
//    public void setDeleteDataSource(List<String> deleteDataSource) {
//        mProvider.setDeleteDataSource(deleteDataSource);
//    }
